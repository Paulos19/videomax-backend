...
export function ContinueWatchingCard({ item }: { item: ContinueWatchingData }) {
  ...
  return (
    <div
      onClick={handlePlay}
      className="group cursor-pointer bg-[#09090D] border border-[#222] hover:border-[#FF5A00] transition-all flex flex-col justify-between overflow-hidden"
    >
      {/* Thumbnail Header */}
      <div className="relative aspect-video w-full bg-[#050508] overflow-hidden border-b border-[#222]">
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <div className="w-10 h-10 bg-[#FF5A00] flex items-center justify-center text-black shadow-[0_0_15px_rgba(255,90,0,0.5)]">
      ...
      {/* Info */}
      <div className="p-3">
        <h4 className="text-[11px] font-mono font-bold text-white uppercase group-hover:text-[#FF5A00] transition-colors truncate">
          {item.title}
        </h4>
        <div className="flex items-center justify-between text-[9px] font-mono text-[#777] mt-1">
