...
import {
  ...
  Mail, Play, Loader2, Sparkles, Clock
} from 'lucide-react'
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 lg:px-6 py-8 space-y-8">
      ...
      <div className="bg-room-surface border border-room-border rounded-2xl p-6 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl bg-room-accent/10 flex items-center justify-center">
            <UserPlus className="w-5 h-5 text-room-accent" />
          </div>
          <div>
            <h2 className="text-room-text font-bold text-lg">Adicionar novo amigo</h2>
            <p className="text-room-text-secondary text-xs">Busque por nome de usuário único ou e-mail cadastrado</p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        {/* Navigation Tabs */}
        <div className="flex border-b border-room-border gap-2">
          <button
            ...
            {friends.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {friends.map((friend) => (
                  <div
                    key={friend.id}
                    className="bg-room-surface border border-room-border rounded-2xl p-4 flex items-center justify-between gap-3 hover:border-room-accent/30 transition-all group"
                  >
